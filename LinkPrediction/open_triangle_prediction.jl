## Code taken from https://github.com/arbenson/ScHoLP-Tutorial for accurate comparison to previous literature
...
basename_str(dataset::String) = "$(OUTDIR)/$dataset-open-tris-$(mid_split)-$(end_split)"
...
function collect_local_scores(dataset::HONData)
    ...
    old_simplices, old_nverts =
        split_data(dataset.simplices, dataset.nverts, dataset.times, mid_split, end_split)[1:2]
...
function collect_generalized_means(dataset::HONData)
    triangles, labels = read_data(dataset, mid_split, end_split)
    old_simplices, old_nverts =
        split_data(dataset.simplices, dataset.nverts, dataset.times, mid_split, end_split)[1:2]
...
Collects the open triangles in the first (mid_split)% of the data as well as a label of
...
function collect_labeled_dataset(dataset::HONData)
    ...
    old_simplices, old_nverts, new_simplices, new_nverts =
        split_data(dataset.simplices, dataset.nverts, dataset.times, mid_split, end_split)
    write_dataset(old_simplices, old_nverts, new_simplices, new_nverts,
                  "open-tris-$(mid_split)-$(end_split)")
    train_simplices, train_nverts, val_simplices, val_nverts =
        split_data(dataset.simplices, dataset.nverts, dataset.times, start_split, mid_split)
    write_dataset(train_simplices, train_nverts, val_simplices, val_nverts,
                  "open-tris-$(start_split)-$(mid_split)")
end;
